    ...
    ICCIPDVNAdapter
{
        ...
        returns (uint256 totalFee)
    {
        ...
        ICCIPDVNAdapterFeeLib.Param memory feeLibParam = ICCIPDVNAdapterFeeLib
            .Param(
                _param.dstEid,
    ...
    function ccipReceive(Client.Any2EVMMessage calldata _message) external {
        if (msg.sender != address(router))
            revert CCIPDVNAdapter_InvalidRouter(msg.sender);
    ...
    function getFee(
        ...
        ICCIPDVNAdapterFeeLib.Param memory feeLibParam = ICCIPDVNAdapterFeeLib
            .Param(_dstEid, _confirmations, _sender, defaultMultiplierBps);
        ...
        totalFee = ICCIPDVNAdapterFeeLib(workerFeeLib).getFee(
            feeLibParam,
            config,
            message,
            _options,
    ...
    function _createCCIPMessage(
        ...
        message = Client.EVM2AnyMessage({
            ...
            extraArgs: Client._argsToBytes(
                Client.EVMExtraArgsV1({gasLimit: _gas})
            ),
    ...
    function _assertPeer(
        ...
        if (keccak256(_sourceAddress) != keccak256(sourcePeer)) {
            revert CCIPDVNAdapter_UntrustedPeer(
